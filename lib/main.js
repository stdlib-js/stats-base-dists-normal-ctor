...
function Normal() {
	...
	if ( arguments.length ) {
		...
		if ( !isNumber( mu ) || isnan( mu ) ) {
			throw new TypeError( format( '1618p,HM', mu ) );
		}
		if ( !isPositive( sigma ) ) {
			throw new TypeError( format( '1617c,HN', sigma ) );
		}
	} else {
		mu = 0.0;
		sigma = 1.0;
	...
	defineProperty( this, 'mu', {
		...
		'set': function set( value ) {
			if ( !isNumber( value ) || isnan( value ) ) {
				throw new TypeError( format( '1618d,H8', value ) );
			}
			mu = value;
		}
	});
	defineProperty( this, 'sigma', {
		...
		'set': function set( value ) {
			if ( !isPositive( value ) ) {
				throw new TypeError( format( '1618k,HE', value ) );
			}
			sigma = value;
		}
	});
